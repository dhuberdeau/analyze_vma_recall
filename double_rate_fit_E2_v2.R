...
fit_exp <- function(dat, typ){
  ...
  ## for 3-rate
  fit_prelim <- nlm(f, c(1,-.5, 1, -.8), y = y[ty == typ], tr = tr[ty == typ])
  ...
  ## double rate using nls:
  # y ~ beta1*(1 - exp(beta2*tr)),
  beta_prelim <- fit_prelim$estimate
  fit <- nls(y ~ (1 - beta1*exp(beta2*tr) - beta3*exp(beta4*tr)), 
             data = data.frame(y = y[ty == typ], tr = tr[ty == typ]),
             start = list(beta1 = beta_prelim[1], beta2 = beta_prelim[2], beta3 = beta_prelim[3], beta4 = beta_prelim[4]),
...
fit_struct_double <- list()
file_nums <- c(1,2,3) #exclude 4 b/c of failure to converge in nls
for (i_group in file_nums){
  ...
  s_2 <- summary(fit_2)
  beta_fit <- c(s_2$coefficients[1,1], s_2$coefficients[2,1], s_2$coefficients[3,1], s_2$coefficients[4,1])
  # y_hat_2 <- f_exp2(fit_2$estimate, t)
  # y_hat_2 <- f_exp2(beta_fit, t)
  # y_hat_2 <- beta_fit[1]*(1 - exp(beta_fit[2]*t)) 
  y_hat_2 <- (1 - beta_fit[1]*exp(beta_fit[2]*t) - beta_fit[3]*exp(beta_fit[4]*t))
  
  mat_out = data.frame(t, y_hat_2)
  writeMat(paste("exp_fit_double_E2_", i_group, ".mat",sep=''), mat_out = mat_out)
  
  fit_struct_double <- list.append(fit_struct_double, fit_2)  
